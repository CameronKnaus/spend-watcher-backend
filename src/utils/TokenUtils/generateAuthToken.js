...
// Only one of these parameters is required. Pass 'user' if you already have a
// user object returned from services
// Pass prevToken (JWT as a string) if you have an existing token and would
// like to generate another one from the previous token
// Pass username (string) if you have neither
// Returns null if user can't be found by the given username
module.exports = async function generateAuthToken(prevToken, username, user) {
    if (!user) {
